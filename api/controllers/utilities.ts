...
export const verifyPhone = async (user, phone, carrier) => {
  const phoneData = user['https://brphones'] &&
    user['https://brphones'].find(p => p.phone === phone && p.carrier === carrier)
  ...
  return phoneData && phoneData.verified
    ...
    : auth0.getUser({ id: user.sub })
      ...
      .catch(err => {
        errorEmail({ err, message: `Error verifying ${phone}`, subject: 'Phone verification error' })
        return { hasPhone: false, verified: false }
      })
}

export const verifyEmail = async (user, email) => {
  const emailData = user['https://bremails'] &&
  ...
  return emailData && emailData.verified
    ...
    : auth0.getUser({ id: user.sub })
      .then(u => {
        const freshEmailData = u.user_metadata.emails && u.user_metadata.emails.find(e => e.email === email)
      ...
      .catch(err => {
        errorEmail({ err, message: `Error verifying ${email}`, subject: 'Email verification error' })
        return { hasEmail: false, verified: false }
      })
}

export const deletePlan = (planId: string, userId: string): Promise<DBPlan> => new Promise((resolve, reject) => {
  return Plans.findOneAndRemove({ _id: planId, userId }, (err: Error, result: DBPlan) => err ? reject(err) : resolve(result))
})

export const findPlansByIdentifiers = (identifiers: any): Promise<DBPlan[]> => new Promise((resolve, reject) => {
  Plans.find(identifiers, (err: Error, plans: DBPlan[]) => err ? reject(err) : resolve(plans))
})

export const getPhoneFromString = string => {
  return typeof string === 'string' && Array.from(string).reduce((phone, char) => char.match(/\d/) ? phone + char : phone, '')
}
...
export const generateCode = length => Array(length || 6).fill(10).map(ten => Math.floor(ten * Math.random())).join('')
...
export const isValidJSON = string => {
  try {
    ...
    errorEmail({ subject: 'Invalid JSON', err, message: `Attempted to parse: ${string}\n\n\nResult:` })
...
export const referenceFromString = string => {
  if (string) {
    const book = string.split('').filter((c, i) => !i || !c.match(/[0-9-:]/)).join('')
