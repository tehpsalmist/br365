...
export default function (readingArray, plan) {
  return Promise.all<VerseData>(
    readingArray.map((reading) => getScriptureJSON(reading, plan.translation))
  )
    .then((verseData: VerseData[]) => {
      const fullTranslation = translations.find(
        (t) => plan.translation === t.code
      )
      ...
      const htmlArray = [
        ...
        ...verseData.map((item) => {
          if (!item)
            return "<p>An unfortunate error occurred while formatting this chapter.<br>Visit your dashboard to attempt to resend today's reading.</p>"
          ...
          item.verses.forEach(({ verse, text }) => {
            chapterString += `${
              verse === 1
                ? ''
                : `<sup style="line-height: .75em; font-size: .75em; vertical-align: top; color: green;">${verse}</sup>`
            }${text} `
          })
      ...
      const nextReference =
        bibleArray[
          (bibleArray.findIndex(
            (ref) => ref === readingArray[readingArray.length - 1]
          ) +
            1) %
            1189
        ].split(':')[0]
      ...
      return sendEmail({
        to: plan.email,
        subject: `${plan.planName} (${fullTranslation.acronym}): ${
          months[now.getMonth()]
        } ${now.getDate()}, ${now.getFullYear()}`,
        html: `${finalHTML}<br><br>${nextChapterButton(
          plan.translation,
          nextReference
        )}<br><br>${visitDashboardButton(plan._id)}`,
      })
    })
    .catch((err) =>
      errorEmail({
...
export function getScriptureJSON(
  reading,
  translation,
  iteration = 0
): Promise<VerseData> {
  ...
  return fetch(
    ...
    .then((response) => {
      if (!response.ok) {
        return (
          iteration < 5 && getScriptureJSON(reading, translation, ++iteration)
        )
      }
